...
class ModelsTests(TestCase):
    ...
    def test_comment_str(self):
        comment = Comment.objects.create(
            owner=self.user,
            lot=self.lot,
            text="some very long test_text"
        )
        self.assertEqual(
            str(comment),
            f"{comment.owner}: {comment.text}. "
            f"Lot: {comment.lot}"
        )
    ...
    def test_lot_str(self):
        ...
        self.assertEqual(
            str(lot),
            f"Lot: {lot.name}. Owner: {lot.owner}. Status: {active}"
        )
    ...
    def test_bid_str(self):
        ...
        self.assertEqual(
            str(bid),
            f"User: {bid.user} (lot: {bid.lot.name}, amount: {bid.amount})"
        )
